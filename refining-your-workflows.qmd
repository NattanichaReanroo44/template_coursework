...
author: "06059431 Nattanicha Reanroo"
format:
  html:
    toc: true
    highlight: tango
...
3. understand how and when to show code within reports
4. use style guides and directory templates to provide a consistent structure across projects
This table shows the first 6 rows of penguins dataset
```{r}
head(palmerpenguins::penguins)
```


This graph represents the scatter plot between flipper length and body mass of penguins dataset separated by species
```{r, echo =FALSE, warning=FALSE}
data1 = palmerpenguins::penguins
p = ggplot(data1, aes(x=flipper_length_mm, y=body_mass_g, shape=species, color=species)) + geom_point() + ggtitle("scatter plot of flipper length vs. body mass")
p = p + geom_point(aes(x = mean(data1$flipper_length_mm[data1$species=="Adelie"],na.rm=T), y = mean(data1$body_mass_g[data1$species=="Adelie"],na.rm=T)), color = "red", shape = 8, size = 4)
p = p + geom_point(aes(x = mean(data1$flipper_length_mm[data1$species=="Chinstrap"],na.rm=T), y = mean(data1$body_mass_g[data1$species=="Chinstrap"],na.rm=T)), color = "green", shape = 8, size = 4)
p = p + geom_point(aes(x = mean(data1$flipper_length_mm[data1$species=="Gentoo"],na.rm=T), y = mean(data1$body_mass_g[data1$species=="Gentoo"],na.rm=T)), color = "blue", shape = 8, size = 4)
print(p)
```
